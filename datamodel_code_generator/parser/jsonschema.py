...
class JsonSchemaParser(Parser):
    ...
    def parse_ref(self, obj: JsonSchemaObject, path: List[str], ref) -> None:
        if obj.ref:
            ...
            if (not reference or not reference.loaded) and obj.ref not in ref:
                ...
                if obj.ref.startswith('#'):
                    ...
                    self.model_resolver.add_ref(obj.ref, actual_module_name='').loaded = True

        if obj.items:
            if isinstance(obj.items, JsonSchemaObject):
                self.parse_ref(obj.items, path, tuple(ref))
            else:
                for item in obj.items:
                    self.parse_ref(item, path, tuple(ref))
        if isinstance(obj.additionalProperties, JsonSchemaObject):
            self.parse_ref(obj.additionalProperties, path, tuple(ref))
        ...
        if obj.properties:
            for value in obj.properties.values():
                self.parse_ref(value, path, tuple(ref))

    def parse_raw_obj(self, name: str, raw: Dict[str, Any], path: List[str], ref) -> None:
